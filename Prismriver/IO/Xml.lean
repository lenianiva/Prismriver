/- Adapted from Lean XML -/
module

public import Std.Data.TreeMap.Basic
public import Std.Internal.Parsec

namespace Prismriver.Xml
...
deriving instance BEq for Attributes
deriving instance BEq for Content, Element

private def indentLead (nLevels : Nat) := nLevels.repeat (· ++ "\t") ""

mutual
private partial def eToString (nLevels : Nat) : Element → String
| Element.Element n a c =>
  let lead := indentLead nLevels
  match c with
  | #[] => s!"{lead}<{n}{a}></{n}>"
  -- single string entry doesn't need newline
  | #[.Character c] => s!"{lead}<{n}{a}>{c}</{n}>"
  | c =>
    let components := String.join $ c.toList.map λ content => (cToString (nLevels + 1) content) ++ "\n"
    s!"{lead}<{n}{a}>\n{components}{lead}</{n}>"

private partial def cToString (nLevels : Nat) : Content → String
| Content.Element e => eToString nLevels e
| Content.Comment c => s!"{indentLead nLevels}<!--{c}-->"
| Content.Character c => s!"{indentLead nLevels}{c}"

end
instance : ToString Element := ⟨private_decl% eToString 0⟩
instance : ToString Content := ⟨private_decl% cToString 0⟩
